...
function UpdatePlayerTable(connectedPlayers)
	...
	local Ensihoito, Poliisi, Taksi, Customs, Mekaanikko, players = ('X'), ('X'), ('X'), ('X'), ('X'), 0
	...
	for k,v in pairs(connectedPlayers) do
		...
		if v.job == 'Ensihoito' then
			Ensihoito = "<span style='color: #00ff2a;'>✓</span>"
		elseif v.job == 'Poliisi' then
			police = police + 1
			
			if police >= 1 and police <= 2 then
				Poliisi = "<span style='color: #fff;'>1+</span>"
			elseif police >= 3 and police <= 5 then
				Poliisi = "<span style='color: #fff;'>3+</span>"
			elseif police >= 6 and police <= 7 then
				Poliisi = "<span style='color: #fff;'>6+</span>"
			elseif police >= 8 then
				Poliisi = "<span style='color: #fff;'>8+</span>"
			end
		elseif v.job == 'Taksi' then
			Taksi = "<span style='color: #00ff2a;'>✓</span>"
		elseif v.job == 'Customs' then
			Customs = "<span style='color: #00ff2a;'>✓</span>"
		elseif v.job == 'Mekaanikko' then
			Mekaanikko = "<span style='color: #00ff2a;'>✓</span>"
		end
	end
	...
	SendNUIMessage({
		...
		jobs   = {Ensihoito = Ensihoito, Poliisi = Poliisi, Taksi = Taksi, Customs = Customs, Mekaanikko = Mekaanikko, player_count = players}
	})
end
